...
class EncoderDataset(Dataset):
    ...
    def collate_dicts(self, batch: List[Dict[str, torch.tensor]]):
        ...
        post_proccessed = {
            ...
            'id': select_by_key(batch, 'id'),
            'coords': torch.stack(select_by_key(batch, 'coords'),  dim=-1),
            'coord_pad': torch.stack(select_by_key(batch, 'coord_pad'), dim=-1),
            'coord_conf': torch.stack(select_by_key(batch, 'coord_conf'), dim=-1),
        }
        return post_proccessed