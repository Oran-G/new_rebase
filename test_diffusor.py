...
class EncodedFastaDatasetWrapper(BaseWrapperDataset):
    ...
    def __getitem__(self, idx):
        ...
        MAXLEN = 271
        if len(self.dataset[idx]['seq']) > MAXLEN:
            start = random.randint(0, len(self.dataset[idx]['seq']) - (MAXLEN+1))
            end = start + MAXLEN
        else:
            start = 0
            end = -1
...
def loss_trans(x, x_pred, dclamp=10):
    distances = torch.abs(x - x_pred)
    distances = torch.clamp(distances, max=dclamp)
    distances = torch.square(distances)
    return torch.mean(distances)
def loss_rot(alpha, alpha_pred):
    l = torch.abs(alpha - alpha_pred)
    l = torch.square(l)
    return torch.mean(l)
def lframe(x, x_pred, alpha, alpha_pred, decay, wtrans, wrot, T):
    return torch.tensor(decay**(self.T - T), device=x.device) * (torch.tensor(wtrans, device=x.device) * loss_trans(x, x_pred) + torch.tensor(wrot, device = alpha.device) * loss_rot(alpha, alpha_pred))
...
class RebaseT5(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if random.randint(0, 1) == 0 or t == self.T:
            ...
            loss = lframe(px0[:, :, :14, :], batch['xyz_27'][:, :, :14, :], alpha_0, torch.cat((output[4].reshape(1, -1, L, 10, 2), alpha_mask_0), dim=-1).reshape(1, -1, L, 30), .99, 1, 1, t)
